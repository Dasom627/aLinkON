/* =========================
   LinkON – Signup & Login Frontend JS (app.js)
   ========================= */

// ★ 같은 포트(3000)에서 열리므로 상대경로 사용
const API = ""; // window.location.origin 을 써도 동일 효과
...
form?.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    password: password?.value, // 서버에서 bcrypt로 해시 저장됨
  };
  ...
  try {
    ...
    if (r.ok){
      ...
      window.location.href = "/HTML/Index/index.html"; // 대/소문자 주의
...
/* =========================
   로그인 (index.html에서만 동작)
   ========================= */
const loginForm = document.getElementById("loginForm");
if (loginForm) {
  const emailEl = document.getElementById("email");       // ✅ '#' 제거
  const pwEl    = document.getElementById("password");    // ✅ '#' 제거
  const errEl   = document.getElementById("loginError");  // ✅ '#' 제거
  const btnEl   = document.getElementById("loginBtn");    // 있으면 사용
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        if (errEl) errEl.textContent = data.error || "로그인에 실패했어요.";
